...
const Product: React.FC<Props> = ({ product, products }) => {
  ...
  return (
    <div>
      {/* ===== Head Section ===== */}
      <Header title={`${product.name} - Terra Balance`} />
...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  try {
    ...
    const transformedProducts: itemType[] = randomProducts.map(
      (randomProduct: apiProductsType) => ({
        id: randomProduct.id,
        name: randomProduct.name,
        price: randomProduct.price,
        img1: randomProduct.img1,
        img2: randomProduct.img2,
        discountPercent: randomProduct.discountPercent,
        description: randomProduct.description,
        stock: randomProduct.stock,
        category: randomProduct.category,
        categoryName: randomProduct.category?.name
      })
    );
